...
# ============ 配置区域 ============
SERVICE_NAME="SenceVoice_noise_reduction"           # 服务名称
RUNTIME_DIR=".sencevoice-noise"           # 运行时文件目录
PYTHON_CMD="python"                 # Python执行命令 (可改为: uv run python, python3, conda run -n myenv python 等)
SCRIPT_NAME="test_noise_wss.py"         # 主脚本文件名
DEFAULT_CUDA_DEVICE=4               # 默认CUDA设备号
ENABLE_NOISE_SUPPRESSION=true       # 是否启用噪声抑制 (True/False)
PORT=50150                          # 服务端口
...
start() {
    ...
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICE ENABLE_NOISE_SUPPRESSION=$ENABLE_NOISE_SUPPRESSION nohup $PYTHON_CMD $SCRIPT_NAME \
        --port $PORT \
...
debug() {
    ...
    echo "使用命令: $PYTHON_CMD"
    echo "按 Ctrl+C 退出"
    CUDA_VISIBLE_DEVICES=$CUDA_DEVICE ENABLE_NOISE_SUPPRESSION=$ENABLE_NOISE_SUPPRESSION $PYTHON_CMD $SCRIPT_NAME \
        --port $PORT \
        --speech_noise_thres $SPEECH_NOISE_THRES
}
